...
if(isset($_POST['update'])){
    ...
    $update = "UPDATE bloodcenters SET bcname='$bcname', bcemail='$bcemail', bcpassword='$bcpassword', bcphone='$bcphone', bccity='$bccity' WHERE id='$id'";
    if ($conn->query($update) === TRUE) {
        $msg= "Your profile is updated successfully.";
        header( "location:../bprofile.php?msg=".$msg);
    } else {
        $error= "Error: " . $sql . "<br>" . $conn->error;
        header( "location:../bprofile.php?error=".$error);
    }
    $conn->close();
}
}else{
